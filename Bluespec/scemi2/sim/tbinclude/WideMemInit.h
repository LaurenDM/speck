...
// DO NOT EDIT
// C++ Class with SceMi Message passing for Bluespec type:  MemTypes::WideMemInit
// Generated on: Sat Mar 12 13:30:01 EST 2016
...
/// C++ class representing the hardware tagged union MemTypes::WideMemInit
/// This class has been automatically generated.
class WideMemInit : public BSVType {
 public:
  ...
  WideMemInit (const SceMiMessageDataInterface *msg, unsigned int &off)
    : the_tag(0)
    , m_InitLoad ()
    , m_InitDone ()  {
    unsigned int tmpoff = off;
    the_tag = msg->GetBitRange (off + 536, 0);
    switch (the_tag) {
      case tag_InitLoad: m_InitLoad = WideMemInitLoad(msg, tmpoff) ; break ;
      case tag_InitDone: m_InitDone = BSVVoid(msg, tmpoff) ; break ;
      default: std::cerr << "bad tag fields in WideMemInit constructor" << std::endl;
    };
    off += 537;
  }
  ...
  unsigned int setMessageData (SceMiMessageDataInterface &msg, const unsigned int off=0) const {
    msg.SetBitRange(off + 536, 0, the_tag);
    switch (the_tag) {
      case tag_InitLoad: m_InitLoad.setMessageData (msg, off); break;
      case tag_InitDone: m_InitDone.setMessageData (msg, off); break;
      default: std::cerr << "bad tag fields in WideMemInit setMessageData" << std::endl;
    }
    return off + 537;
  }
  
  /// overload the put-to operator for WideMemInit
  friend std::ostream & operator<< (std::ostream &os, const WideMemInit &obj) {
    ...
    switch (tag) {
      ...
      default: std::cerr << "bad tag fields in WideMemInit operator<<" << std::endl;
    }
    return os;
  }
  ...
  virtual std::ostream & getBitString (std::ostream & os) const {
    ...
    switch (the_tag) {
      case tag_InitLoad:
        ...
        break;
      case tag_InitDone:
        os << std::setw(536) << std::setfill('0') << '0' ;
        m_InitDone.getBitString (os);
        break;
      default:
        std::cerr << "bad tag fields in WideMemInit::getBitString()" << std::endl;
        os << std::setw(536) << std::setfill('0') << '0' ;
        break;
    }
  return os;
  ...
  virtual BSVType * getMember (unsigned int idx) {
    switch (idx) {
      ...
      default: std::cerr << "Index error in getMember for class WideMemInit" << std::endl ;
    };
    return 0;
  };
  ...
  virtual const char * getMemberName (unsigned int idx) const {
    switch (idx) {
      ...
      default: std::cerr << "Index error in getMemberName for class WideMemInit" << std::endl ;
    };
    return 0;
  };
